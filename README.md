...
- `POST /upload` - **iOSデバイスからのWAV音声ファイルアップロード**
  ...
  **📋 保存方式（2モード対応）**:
  ...
  2. **❌ 自動時刻モード（廃止）**: 
     - **2025/7/9より廃止**: X-File-Pathヘッダーがない場合はエラーを返します
     - **理由**: サーバー側での時刻計算では、実際の録音時刻と異なるフォルダに保存される問題があるため
...
### 2025/7/9 - X-File-Path仕様変更 🔧
- **ファイルパス形式にrawディレクトリ追加**
  - 旧: `device_id/YYYY-MM-DD/HH-MM.wav`
  - 新: `device_id/YYYY-MM-DD/raw/HH-MM.wav`
- **X-File-Pathヘッダーを必須化**
  - ヘッダーがない場合はHTTP 400エラーを返すように変更
  - サーバー側での時刻計算を完全に禁止
- **iOSアプリ側のローカル保存構造も同時に変更**
  - 日付別ディレクトリで管理するように改善
  - 日をまたぐ録音でのファイル重複問題を解決
