...
- Lambda関数はSKIPを**特別扱いせず**、データ欠損の一種として処理
- 累積分析は**全てのケースで必ず実行**（SKIP/失敗/成功に関わらず）
- 処理コストを削減しながら、ダッシュボードの継続性を完全に保証
...
### 2025/11/05 - v3.1.1（SKIP処理の統一化対応）
- **Lambda関数側の処理フロー統一に対応**: SKIPステータスの特別扱いを廃止
- **README.mdの更新**: 動作仕様をLambda関数の新しい処理フローに合わせて修正
- **累積分析の保証**: 全ケースで必ず実行されることを明記
- **06:00問題の解決**: 夜間スキップ後の処理停止問題が根本的に解決
